...
def generate_launch_description():
    ...
    ld.add_action(DeclareLaunchArgument(
            ...
            description=''))
    ld.add_action(IncludeLaunchDescription(
        PathJoinSubstitution([FindPackageShare('hunav_gazebo_wrapper'),'launch','nav2_follow_waypoints.launch.py']),
        launch_arguments={'params_file':LaunchConfiguration('params_file'),
                          'launchnav2can':LaunchConfiguration('launchnav2can')}.items()
    ))
    ld.add_action(IncludeLaunchDescription(
        PathJoinSubstitution([FindPackageShare('hunav_evaluator'),'launch','hunav_evaluator_launch.py']),
